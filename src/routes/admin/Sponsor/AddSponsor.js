...
function AddSponsorData(props) {
  const [form] = Form.useForm();

  const { setFieldsValue, resetFields } = form;
  const { manageAdminRequest, admin_reducer, editDetails } = props;

  const { admin_loading, admin_success, admin_request_type } = admin_reducer;

  function submitSponsorDetails(data) {
    const username = UserInfo.getUser();
    if (!data?.id) {
      manageAdminRequest({
        request_type: "ADD_SPONSOR_DETAILS",
        ...data,
        createdBy: username,
      ...
      manageAdminRequest({
        request_type: "EDIT_SPONSOR_DETAILS",
        ...data,
        changedBy: username,
  ...
  useEffect(() => {
    if (
      admin_request_type === "ADD_SPONSOR_DETAILS" ||
      admin_request_type === "EDIT_SPONSOR_DETAILS"
    ) {
      resetFields();
    }
  ...
  return (
    <Card size="small" title="Add Sponsor Details">
      <Form form={form} labelCol={{ span: 24 }} onFinish={submitSponsorDetails}>
        <Row gutter={20}>
...
export default connect(mapStateToProps, mapDispatchToProps)(AddSponsorData);
